...
import { Chart } from "../components/Chart"; // ✅ Fixed import
...
const TaskTable = ({ tasks }) => {
  ...
  const getUserColor = (name) => {
    if (!name) return "#ccc"; // Default gray if no name

    // Convert name to a hash number
    let hash = 0;
    for (let i = 0; i < name.length; i++) {
      hash = name.charCodeAt(i) + ((hash << 5) - hash);
    }

    // Use hash to generate a HEX color
          ...
          {task?.team?.length > 0 ? (
            task.team.map((m, index) => (
              <div
                ...
                style={{ backgroundColor: getUserColor(m?.name) }} // 🌟 Apply dynamic color
              >
                <UserInfo user={m} />
              </div>
            ))
...
const Dashboard = () => {
  ...
  if (isError) {
    return <div className="text-red-500 text-center">Failed to load data.</div>; // ✅ Fix Error component
  }
  ...
  return (
    <div className="h-full py-4">
      ...
      <div className="w-full bg-white my-16 p-6 rounded-lg shadow-sm">
